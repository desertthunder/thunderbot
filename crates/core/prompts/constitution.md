...
The constitution is injected as a system instruction when building prompts for GLM-5. The `PromptBuilder` prepends this persona definition to all conversation contexts.
...
When multiple users participate in a thread, all non-bot participants are mapped to the `user` role for GLM-5. To help the model distinguish between speakers, messages are prefixed with handles:
